...
const NIFTY50_TICKERS = new Set([
  'ADANIENT','ADANIPORTS','APOLLOHOSP','ASIANPAINT','AXISBANK','BAJAJ-AUTO','BAJFINANCE','BAJAJFINSV',
  'BEL','BHARTIARTL','BRITANNIA','CIPLA','COALINDIA','DRREDDY','EICHERMOT','ETERNAL','GRASIM','HCLTECH',
  'HDFCBANK','HDFCLIFE','HEROMOTOCO','HINDALCO','HINDUNILVR','ICICIBANK','INDUSINDBK','INFY','ITC','JIOFIN',
  'JSWSTEEL','KOTAKBANK','LT','M&M','MARUTI','NESTLEIND','NTPC','ONGC','POWERGRID','RELIANCE','SBILIFE','SBIN',
...
function getFallbackDb(): DbSchema {
  ...
  const companies: CompanyRow[] = (
    ...
    exchanges: item.exchanges || [ ...(item.nseSymbol ? ['NSE' as const] : []), ...(item.bseCode ? ['BSE' as const] : []) ],
    sector: item.sector,
    sectorSlug: sectorSlugMap[item.sector] || 'diversified',
    industry: item.industry,
    description: item.description,
    logoUrl: null,
    isNifty50: NIFTY50_TICKERS.has(item.ticker),
    isActive: true,
